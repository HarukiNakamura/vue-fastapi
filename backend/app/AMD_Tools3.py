...
AMD_Tools3.py
   ...
   22. GetMetData_Hourly:メッシュ農業気象データ（時別値）を取得する関数

    改変履歴：
    20201121 GetMetData_Hourly 関数を追加 （時別値取得用関数） 
...
USER="nakamurah936"
PASSWORDS=["ZbyPGFat2022", "パスワード"] #二つのうち、どちらかが正しければデータは取得できます
...
def xurl2dh(url):
    ...
    if no_pw:
        try: #try local storage
            dh = xr.open_dataset(a) # masks fillvalues (see mask_and_scale)
            return dh,None
        except:
            pass
        ...
        try: #try local storage
            #print("trying",a[:-3])
            dh = xr.open_dataset(a[:-3])
            return dh,None
        except:
            pass

        try: #try old passwordless site
            dh = xr.open_dataset(a[:-3] + "?" + b)
            return dh,None
        except:
            pass
    else:
...
if __name__ == "__main__":
    lalodomain = [ 36.5, 37.5, 137.5, 138.5]
    url = "/big/AMDNEW"
    g = GetGeoData("altitude", lalodomain, url=url, namuni=True)
    timedomain = ['1980-04-01', '1981-08-31']
    lalodomain = [35.0,  35.2, 135.0, 135.0]
    me = GetMetData('TMP_max', timedomain, lalodomain, url=url, namuni=True)
    timedomain = ['2011-04-01', '2011-08-31']
    lalodomain = [35.0,  35.2, 135.0, 135.0]    
    mc = GetMetData('TMP_mea', timedomain, lalodomain, url=url,cli=True, namuni=True)
    url = "/new/AMSPLIT"
    timedomain = ['2040-04-01', '2041-08-31']
    lalodomain = [35.0,  35.2, 135.0, 135.0]    
    s = GetSceData('TMP_max', timedomain, lalodomain, "MIROC5", "RCP2.6", url=url, namuni = True)
